...
| `bmr` | openpyxl | `eidr_core.bmr_io.read_sheet` / `open_sheet` / `write_sheet` / `subset_rows` (lazy). `read_headers` / `family_layout` / `families_for` / `template_for_creation_type` need no extra — they take an already-open worksheet or plain names |
...
| Module | Contents |
|---|---|
| `eidr_core.bmr_io` | Workbook surgery (writer primitives: HEADER_ROW, read/count/rightmost/expand family, transplant, fix_shared_strings) **+** the reader half — `open_sheet` (streaming context manager, lazy rows with absolute sheet row numbers), `read_sheet` (a `list()` over it), `read_headers`, sparse `family_layout`, shared `RepeatPlan`/`pad_groups` **+** the writer composition (0.29.0) — `write_sheet` over rows already mapped to `{column: value}`, `TEMPLATES`/`families_for`/`SCHEMA_MAX` (Template-22 facts, registry maxima from the schema), `template_for_creation_type` (routing), `header_map` (the header-row policy over a plain row). Mapping (what a row says) stays per consumer; the writer reports every value it could not place **+** `subset_rows` (0.30.0) — row-subset COPY of a BMR sheet onto a copy of the source workbook, template resolved from the tab name, conformance to `Template.headers` checked at emit, `blank_columns=` the one sanctioned edit |
| `eidr_core.vendor` | The vendoring mechanism (0.31.0, `specs/vendoring.md`) for packages that cannot depend on eidr-core (the SDK, the CLI tools): `python -m eidr_core.vendor sync|check --config vendor.toml` copies pinned-commit modules, rewrites imports to the consumer's package, refuses a module list not closed under imports, and `check` is the consumer's CI gate against drift. The copy imports with no other eidr-core file present. Stdlib only |
...
| `eidr_core.altidtool_io` | AltIDTool input-line format (variable 3/4/5-col compose, tolerant parse; contract in `specs/altidtool-format.md`) **+** `parse_edit_line` / `AltIdRemoval` (spec v1.1, 0.32.0): the edit-file superset with removal lines **+** `multi_form_domains()` (spec v1.4, 0.39.0): the declared rule-6 exemption list, `specs/multi_form_kinds.json` |
...
| `eidr_core.inheritance` | Full-record construction: what a child record inherits from its parent (schema `inheritedBaseObjectInfoGroup`) plus the four system-generated title patterns. Two shape adapters over one policy — `build_full_base` (registry JSON) and `build_full_record` (record objects) — plus a per-field **`provenance(rec)`** map — `{field: 'self' | 'inherited' | 'system'}`, carried on the returned record and `{}` for a record the builder never touched, so "unknown" stays distinguishable from "self". eidr-core imports neither consumer's model |
| `eidr_core.ids` | EIDR **Content ID** syntax + ISO 7064 Mod 37,36 check character (`check_character`, `is_valid_eidr_id`, `fault`). Party IDs (`10.5237/`) are not covered — see the module docstring **+** `find_content_ids` / `CONTENT_ID_SEARCH_RE` (0.32.0): every content ID mentioned in free text, check-character-validated by default |
