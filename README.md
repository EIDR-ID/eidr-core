...
| `eidr_core.registry` | ✅ **LANDED 0.6.0** — THE SDK client factory (sandbox2 default, credential precedence incl. `.secrets.json` registry section, superparty gate, lazy SDK import) | eidr-wikidata `eidr/registry_client.py` (now a shim); eidr-dq adoption open |
