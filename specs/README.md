...
| `golden-pairs.md` | ✅ **RATIFIED 2026-09-10** — the three fixture shapes: `pair` (scorer; the original), `case` (verdict layer: candidate SETS, assessment/terminal-set consequences, `notes_match`/`notes_not_match` — S-26), and `recovery_pool` (captured mirror pool, invariant is RECALL — S-9). Plus what every fixture must satisfy: mutation-tested, both directions for a gate, one lesson, scope stated, properties over values. | BMR-Review evaluator (`golden.py`); De-Dupe UI JS conformance |
| `vendoring.md` | ✅ **LANDED 2026-09-11 (1.0.0, eidr-core 0.31.0)** — the contract for a package that cannot depend on eidr-core (the SDK, the CLI tools) to carry ONE implementation anyway: `vendor.toml` pins a commit, `python -m eidr_core.vendor sync` copies and rewrites the listed modules (closed under imports, no residual `eidr_core` text), `check` is the consumer's CI gate. Guarantee: the copy imports with no other eidr-core file present. Proposed by python-tools; first consumer python-sdk (`ids`) |
