...
|---|---|---|
| `eidr_core.bmr_io` | Workbook surgery (writer primitives: HEADER_ROW, read/count/rightmost/expand family, transplant, fix_shared_strings) **+** the reader half (parameterized stop rule, sparse `family_layout`, shared `RepeatPlan`/`pad_groups`). Orchestration stays per-consumer by design. | eidr-wikidata `bmr/writer.py` + BMR-Review `audit/bmr_writer.py` (both importers); eidr-dq `flatten.py` folded in |
...
| `eidr_core.ids` | EIDR **Content ID** syntax + ISO 7064 Mod 37,36 check character (`check_character`, `is_valid_eidr_id`, `fault`). Party IDs (`10.5237/`) deliberately not covered — see the module docstring | BMR-Review `validate_returned_ids.py` |
